...
const MiddleBannerDetails = ({id , setMidBanDetCtrl}) => {
    ...
    const removeBanner = async () => {
        try {
            ...
            await axios.delete(`https://mernfa-fileshop-server.iran.liara.run/api/middle-banner/remove-middle-banner/${id}`)
            setRemoveButtonDisabled(false)
            onCloseModal()
            toast.success("بنر میانی با موفقیت حذف شد.",{position: "top-right",autoClose: 5000,rtl: true,className: 'toast-message'})
            setTimeout(() => {
                setMidBanDetCtrl("")
            }, 500);
        } catch (error) {
            toast.error("خطا در حذف بنر میانی",{position: "top-right",autoClose: 5000,rtl: true,className: 'toast-message'})
        }
    }

    const formSubmit = async (e) => {
        e.preventDefault();
        if(image == "" || imageAlt == "" || link == ""){
            return toast.error("خطا در بروزرسانی بنر میانی",{position: "top-right",autoClose: 5000,rtl: true,className: 'toast-message'})
        }
        ...
        try {
            await axios.put(`https://mernfa-fileshop-server.iran.liara.run/api/middle-banner/update-middle-banner/${id}`,formData);
            toast.success("بنر میانی با موفقیت بروزرسانی شد.",{position: "top-right",autoClose: 5000,rtl: true,className: 'toast-message'})
        } catch (error) {
            toast.error("خطا در بروزرسانی بنر میانی",{position: "top-right",autoClose: 5000,rtl: true,className: 'toast-message'})
        }
    ...
    useEffect(()=>{
        goTopCtrl();
        axios.get(`https://mernfa-fileshop-server.iran.liara.run/api/middle-banner/one-middle-banner/${id}`)
        .then((d) => {
                ...
                loading ? (
                    ...
                    <>
                        <div className="flex justify-between items-center mt-4">
                            <h2 className="text-orange-500 text-xl">اطلاعات بنر</h2>
                            <button className="bg-red-500 text-white px-4 py-1 rounded-md transition-all duration-300 hover:bg-red-600" onClick={onOpenModal}>حذف بنر</button>
                            <Modal open={showDialog} onClose={onCloseModal} center>
                              <div>
                                <h1 className="py-10">آیا مطمئن هستید که می خواهید این بنر را حذف کنید؟</h1>
                                <div className="flex flex-row-reverse gap-2">
                                    <button onClick={removeBanner} disabled={removeButtonDisabled} className="flex items-center px-4 py-1 outline-none rounded-md bg-red-500 text-white transition-all duration-300 hover:shadow-[0px_1px_8px_rgba(200,0,0,0.75)]">
                                        حذف
                                        {
