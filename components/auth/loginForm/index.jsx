...
const LoginForm = () => {
  ...
  const onSubmitForm = async (data) => {
    ...
    try {
      const result = await axios.post("https://mernfa-fileshop-server.iran.liara.run/api/user/login-user",data)
      ...
      console.log(error);
      toast.error(error.response?.data?.msg ? error.response.data.msg : "خطا در ورود به حساب",{position: "top-right",autoClose: 5000,rtl: true,className: 'toast-message'})
      setDisabledButton(false); 
    }
  }
    ...
    <section className="container mx-auto flex justify-center">
      <form onSubmit={handleSubmit(onSubmitForm)} className="w-full sm:w-96 flex flex-col gap-6 my-16 mx-4 xs:mx-8 px-4 py-6 rounded-lg shadow-[0px_0px_8px_rgba(0,0,0,0.25)]">
        <div className="flex justify-center items-center gap-4">
          <h1 className="text-base sm:text-lg text-center text-blue-400">ورود به حساب</h1>
          <Link href={"/register"} className="bg-blue-500 text-white p-2 rounded-md text-base sm:text-sm">ثبت نام در سایت</Link>
        </div>
